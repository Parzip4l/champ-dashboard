@extends('layouts.vertical', ['title' => 'Maintenance Schedule List'])
            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex align-items-center justify-content-between">
                        ...
                        <a href="{{ route('maintenance.schedule.create') }}" class="btn btn-sm btn-soft-primary">
                ...
                <div id="table-search">
                    <table class="table mb-0">
                        ...
                        <tbody id="user-table-body">
                            ...
                            <tr>
                                <td class="ps-3">{{ $schedule->item->name ?? 'No Item' }}</td>
                                <td><a href="" class="text-primary">{{ $schedule->schedule }}</a></td>
                                <td>{{ $schedule->next_maintenance }}</td>
                                <td>
                                    <div class="d-flex gap-2">
                                        <a href="{{ route('maintenance.schedule.edit', $schedule->id) }}" class="btn btn-soft-primary btn-sm">
                                            <iconify-icon icon="solar:pen-2-broken" class="align-middle fs-18"></iconify-icon>
                                        </a>
                                        <a href="#!" class="btn btn-soft-danger btn-sm" onclick="confirmDelete({{ $schedule->id }})">
                    ...
                    <tfoot>
                        <div class="d-flex justify-content-between mx-3 mt-2 mb-2">
                            ...
                            <div class="">
                                {{ $schedules->links('pagination::bootstrap-4') }}  <!-- Pagination links -->
                            </div>
                        </div>
                    </tfoot>
                </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#search-input, #status').on('change keyup', function() {
            ...
            $.ajax({
                url: "{{ route('maintenance.schedule.index') }}",  // Route for user list
                method: 'GET',
                data: { 
    ...
    function confirmDelete(ScheduleId) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                fetch('/maintenance/schedule/' + ScheduleId, {
                    method: 'DELETE',
                    headers: {
